...
app.get('/health', (req, res) => {
    res.json({ status: 'ok', message: 'Halal Trading Bot with Multi-Endpoint Fallback' });
});
...
// MULTI-ENDPOINT BINANCE API WITH FALLBACK
class BinanceAPI {
    ...
    static async getServerTime() {
        ...
        for (const baseUrl of allEndpoints) {
            try {
                const timeEndpoint = baseUrl.includes('/api/') ? baseUrl : `${baseUrl}/api/v3/time`;
                const response = await axios.get(timeEndpoint, { timeout: 3000 });
                console.log(`✅ Time sync successful via: ${baseUrl}`);
                return response.data.serverTime;
            } catch (error) {
                console.log(`⚠️ Time endpoint failed: ${baseUrl}`);
                continue;
            }
        }
    ...
    // CORE MULTI-ENDPOINT REQUEST METHOD
    static async makeRequest(endpoint, method, apiKey, secret, params = {}, useTestnet = false) {
        try {
            ...
            if (error.response) {
                ...
                if (data.code === -1021) {
                    console.log('⏰ Timestamp error - syncing time and retrying...');
                    await this.delay(1000);
                    throw new Error('Timestamp error - retry');
                }
                ...
                if (status === 451) {
                    throw new Error(`LOCATION RESTRICTED: All Binance endpoints blocked. Use Testnet mode or a VPS.`);
                }
                
                if (data.code === -2014 || data.code === -2015) {
                    throw new Error('Invalid API key format or permissions. Check your Binance API settings.');
                }
...
app.listen(PORT, '0.0.0.0', () => {
    console.log('\n' + '='.repeat(60));
    console.log('🌙 HALAL AI TRADING BOT - MULTI-ENDPOINT FALLBACK VERSION');
    console.log('='.repeat(60));
    console.log(`✅ Server running on port: ${PORT}`);
    console.log(`✅ Multi-endpoint fallback active`);
    console.log(`✅ Trying endpoints: api, api1, api2, api3, api4, data`);
